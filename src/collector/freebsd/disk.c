 ...
 * library owns dinfo->mem_ptr across calls — we don't free it; the
 * libdevstat pool is reused on the next call and released only when
 * libdevstat tears itself down at process exit.
 */
