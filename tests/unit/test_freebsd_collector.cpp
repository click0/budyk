...
int main() {
    ...
    //    valid percentage in [0,100], cpu.count > 0, ctx remembers state.
    {
        ...
        assert(s1.cpu.total_percent == 0.0);
    ...
    // 6. Network — same timestamp = no divide-by-zero.
    {
        budyk_net_ctx_c ctx;
        std::memset(&ctx, 0, sizeof(ctx));
        budyk_sample_c s;
        ...
        assert(budyk_collect_network_freebsd(&ctx, &s) == 0);

        budyk_sample_c s2;
        std::memset(&s2, 0, sizeof(s2));
        s2.timestamp_nanos = 5'000'000'000ULL;
        assert(budyk_collect_network_freebsd(&ctx, &s2) == 0);
        assert(s2.net.rx_bytes_per_sec == 0);
        assert(s2.net.tx_bytes_per_sec == 0);
    }

    // 7. Disk — baseline + delta.
    {
    ...
    // 8. Disk — same timestamp = no divide-by-zero.
    {
        budyk_disk_ctx_c ctx;
        std::memset(&ctx, 0, sizeof(ctx));
        budyk_sample_c s;
        std::memset(&s, 0, sizeof(s));
        s.timestamp_nanos = 5'000'000'000ULL;
        assert(budyk_collect_disk_freebsd(&ctx, &s) == 0);
